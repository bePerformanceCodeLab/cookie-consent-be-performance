...
describe("stringifyCookie", () => {
    it("should return a string with the correct name and value", () => {
        ...
        expect(result).toContain(`${encodeURIComponent(name)}=${encodeURIComponent(value)}`);
    });
    ...
    it("should include the expires attribute if provided as a number (number of days)", () => {
        ...
        const expectedDate = new Date(Date.now() + expires * MILLISECONDS_IN_DAY).toUTCString();
...
//# sourceMappingURL=safeCookie.test.js.map
