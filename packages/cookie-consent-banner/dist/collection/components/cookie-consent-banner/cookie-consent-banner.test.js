...
describe("Cookie Consent Banner", () => {
    let page;
    const clickInCookieBanner = async (innerSelector) => {
        ...
        await page.evaluate((inner) => {
            document
                ...
                ?.shadowRoot?.querySelector(inner)?.click();
        }, innerSelector);
    };
    const getConsentCookie = async () => (await page.cookies()).find((cookie) => cookie.name === "cookies_accepted_categories");
    beforeEach(async () => {
    ...
    it("should be displayed if no cookies are set", async () => {
        await page.setContent(cookieBannerFullyConfigured);
        const cookieBannerInnerDiv = await page.find("cookie-consent-banner >>> .cc");
    ...
    it("should set cookies_accepted_categories to all passed cookie categories when 'Accept All' button is clicked", async () => {
        ...
        await page.waitForChanges();
        const acceptAllBtn = await page.find("cookie-consent-banner >>> .btn_accept_all");
        ...
        const cookieAcceptedCategories = (await page.cookies()).find((cookie) => cookie.name === "cookies_accepted_categories");
        expect(cookieAcceptedCategories?.value).toBe(encodeURIComponent("technically_required,analytics"));
    });
    it("should set cookies_accepted_categories to only the mandatory when 'Only required' button is clicked", async () => {
        ...
        await page.waitForChanges();
        const onlyRequiredButton = await page.find("cookie-consent-banner >>> .btn_essentials_only");
        ...
        expect(cookieAcceptedCategories?.value).toBe(encodeURIComponent("technically_required"));
    });
    it("should set the cookies_accepted_categories cookie with all passed cookie attributes", async () => {
        ...
        await page.$eval("cookie-consent-banner", 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (elm, _inAWeekTimestamp) => {
